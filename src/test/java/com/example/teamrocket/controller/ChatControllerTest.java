...
public class ChatControllerTest {
    ...
    @Test
    void listRoomSuccess_NoSearchCondition() throws Exception{
        ...
        given(chatService.listRoom(isNull(),isNull(),any())).willReturn(
                result);

        mockMvc.perform(get("/api/v1/chat/room-list?page=0&size=5"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.result.firstPage").value(false))
                .andExpect(jsonPath("$.result.lastPage").value(true))
    ...
    @Test
    void listRoomFail_invalidSearchCondition() throws Exception{

        doThrow(new ChatRoomException(INVALID_SEARCH_CONDITION))
                .when(chatService).listRoom(anyString(),isNull(),any());


        mockMvc.perform(get("/api/v1/chat/room-list?page=0&size=5&rcate1=서울시"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.result").isEmpty())
                .andExpect(jsonPath("$.errorMessage").value(INVALID_SEARCH_CONDITION.getMessage()))
                .andDo(print());
    }
